...
import { ChevronLeft, MoreHorizontal, CheckCircle2, ChevronRight, Store } from 'lucide-react';

export default function LargeFreightService() {
  const [useEcoBox, setUseEcoBox] = useState<boolean>(false);
  ...
  return (
    <div className="flex flex-col h-full bg-[#F4F5F7] absolute inset-0 z-10 font-sans">
      <div className="bg-white flex items-center justify-between px-4 py-3 sticky top-0 z-20">
        ...
        <h1 className="text-[17px] font-bold text-gray-900">大票直达</h1>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 pb-32">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm space-y-4">
           <div className="flex items-center justify-between border-b border-gray-50 pb-4 cursor-pointer group">
              <div className="flex items-center">
                 <div className="bg-blue-100 text-blue-600 w-9 h-9 rounded-full flex items-center justify-center font-bold text-[14px] mr-3">收</div>
                 <div>
                   <div className="text-gray-900 font-bold text-[16px] mb-0.5">收件人信息</div>
                   <div className="text-gray-400 text-[13px]">请输入收件人信息</div>
                 </div>
              </div>
              <span className="text-gray-300 text-[13px] group-hover:text-blue-500 transition-colors">地址簿</span>
           </div>
           <div className="flex items-center justify-between cursor-pointer group">
              <div className="flex items-center">
                 <div className="bg-yellow-100 text-yellow-500 w-9 h-9 rounded-full flex items-center justify-center mr-3 rounded-bl-none"><Store size={18} /></div>
                 <div>
                   <div className="text-gray-900 font-bold text-[16px] mb-0.5">线下网点自提</div>
                   <div className="text-gray-400 text-[13px]">请选择提货网点</div>
                 </div>
              </div>
              <span className="bg-red-50 text-red-500 text-[10px] px-1.5 py-0.5 rounded border border-red-100 font-medium">官方推荐</span>
           </div>
        </div>
        ...
        {/* CARD HỘP GỖ - Tương tác */}
        <div className={`bg-white rounded-2xl p-4 shadow-sm border-[1.5px] transition-all duration-300 ${useEcoBox ? 'border-blue-400 shadow-blue-100' : 'border-transparent'}`}>
          <div className="flex justify-between items-center py-1 mb-3">
            <span className="text-gray-900 text-[15px] font-bold">大件包装 <span className="text-gray-400 font-normal text-[12px] ml-1">可循环使用1-2次</span></span>
            <div 
              onClick={() => setUseEcoBox(!useEcoBox)} 
              className={`flex items-center cursor-pointer rounded-full pl-2 pr-1 py-1 transition-all border ${useEcoBox ? 'bg-blue-50 border-blue-200' : 'bg-[#F8F9FA] border-transparent hover:bg-gray-100'}`}
            >
               <span className={`text-[13px] font-bold mr-1.5 ${useEcoBox ? 'text-blue-600' : 'text-gray-500'}`}>¥300/m³</span>
               <div className={`w-[18px] h-[18px] rounded-full border flex items-center justify-center ${useEcoBox ? 'bg-blue-600 border-blue-600' : 'bg-white border-gray-300'}`}>
                 {useEcoBox && <CheckCircle2 size={14} className="text-white" />}
               </div>
            </div>
          </div>

          <div className="w-full h-24 bg-gradient-to-r from-[#E0F7FA] to-[#D1FAE5] rounded-xl flex flex-col items-center justify-center border border-[#BCEBEA] relative overflow-hidden">
             <div className="z-10 text-[#047857] font-bold text-[20px] tracking-widest italic drop-shadow-sm">安全包装放心承运</div>
             <div className="z-10 text-[#047857] text-[10px] px-3 py-0.5 rounded-full mt-1 bg-[#047857]/10 font-medium">交给极光转运</div>
             <div className="absolute right-4 bottom-1 text-5xl opacity-70 drop-shadow-sm">🚄</div>
             <div className="absolute left-2 top-2 text-4xl opacity-30">✈️</div>
          </div>
        </div>

        {/* CARD BẢO HIỂM TRỐNG (Có Nút Radio Chụp ảnh) */}
      ...
      <div className="bg-white px-5 py-3 absolute bottom-0 left-0 right-0 z-30 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] border-t border-gray-100 flex items-center justify-between">
        <div>
           <div className="text-[15px] text-gray-800 font-medium">预估 <span className="text-blue-600 font-bold text-xl ml-1">{useEcoBox ? '¥ 300' : '--'}</span></div>
           <div className="text-[11px] text-gray-400 mt-0.5">最终费用以仓库称重后开单为准</div>
        </div>
        <button className={`px-9 py-3 rounded-full text-[16px] font-medium transition-all ${agreed ? 'bg-blue-600 text-white shadow-md active:scale-95' : 'bg-blue-400 text-white opacity-80 cursor-not-allowed'}`}>
          立即下单
        </button>
      </div>
    </div>
