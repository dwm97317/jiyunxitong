  ...
  return (
    <div className="space-y-4">
      ...
      <section className="bg-white rounded-2xl p-5 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold">工号：9527</h3>
              <p className="text-xs text-gray-400">电话：010-80908803</p>
            </div>
          </div>
          <motion.button
            whileTap={{ scale: 0.95 }}
        ...
        <div className="bg-[#EBF3FF] rounded-xl p-3">
          <p className="text-[11px] text-[#4A90E2] leading-relaxed">
            若客服电话无法接通，可拨打极光转运官方电话95353，体验您的理赔专属服务哦
          </p>
        </div>
        ...
        <div className="flex justify-between items-start pt-2 px-2">
          {[
            { label: '发起申请', active: true },
            { label: '审核中', active: true },
            { label: '受理中', active: true },
            { label: '理赔完成', active: claim.status === 'completed' },
          ].map((step, idx, arr) => (
            <div key={idx} className="flex flex-col items-center gap-2 flex-1 relative">
              <div className={`w-5 h-5 rounded-full border-4 flex items-center justify-center z-10 ${
                step.active ? 'border-blue-100 bg-[#4A90E2]' : 'border-gray-100 bg-white'
              }`}>
      ...
      <section className="bg-white rounded-2xl p-5 shadow-sm space-y-6">
        <h3 className="text-base font-bold">理赔进度</h3>
          ...
          {claim.status === 'completed' ? (
            <>
              ...
              <div className="relative">
                ...
                <div className="space-y-1">
                  <h4 className="text-base font-bold text-[#4CAF50]">理赔完成</h4>
                  <p className="text-sm text-gray-400">2022-04-02 14:38</p>
                </div>
              </div>
              {/* Previous Statuses */}
              {[
                { status: '受理中', time: '2022-04-02 14:38' },
                { status: '核实中', time: '2022-04-02 09:53' },
                { status: '发起理赔申请', time: '2022-04-02 09:53' },
              ].map((item, idx) => (
            ...
            <>
              ...
              <div className="relative">
                ...
                <div className="space-y-1">
                  <h4 className="text-base font-bold text-[#4A90E2]">受理中</h4>
                  <p className="text-sm text-gray-400">2022-04-02 14:38</p>
                </div>
              </div>
              {/* Previous Statuses */}
              {[
                { status: '核实中', time: '2022-04-02 14:38' },
                { status: '发起理赔申请', time: '2022-04-02 09:53' },
              ].map((item, idx) => (
                <div key={idx} className="relative">
                  <div className="absolute -left-6.5 top-1.5 w-4 h-4 bg-white rounded-full border border-gray-200 flex items-center justify-center z-10">
