...
import { 
  ...
  Settings, Send, Search, Gift, User, Wallet, MapPin, Package, Headphones,
  Copy, Edit3, Crown, Shield
} from 'lucide-react';
  ...
  return (
    <div className="pb-20 bg-gray-50 min-h-screen">
      ...
      <div className="px-4 -mt-20 relative z-10 space-y-4">
        ...
        {/* Invite Banner */}
        <Link to="/invite" className="bg-gradient-to-r from-gray-800 to-gray-900 rounded-xl p-3 flex items-center justify-between text-white shadow-lg block">
          <div className="flex items-center space-x-3">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm flex justify-around text-center">
          ...
          <Link to="/address" className="flex flex-col items-center space-y-2">
            <MapPin size={28} className="text-blue-500" strokeWidth={1.5} />
            <span className="text-xs text-gray-700">地址簿</span>
          </Link>
          <Link to="/wallet" className="flex flex-col items-center space-y-2">
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <MenuItem icon={<ShieldCheck size={20} className="text-gray-600" />} title="在线理赔" />
          <MenuItem icon={<UserCheck size={20} className="text-gray-600" />} title="实名认证" to="/auth" />
          <MenuItem icon={<Mail size={20} className="text-gray-600" />} title="绑定邮箱" to="/email" />
          <MenuItem icon={<CreditCard size={20} className="text-gray-600" />} title="申请代付" hasBorder={false} />
        </div>

        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <MenuItem icon={<PackageSearch size={20} className="text-gray-600" />} title="包裹认领" />
          <MenuItem icon={<PackageX size={20} className="text-gray-600" />} title="包裹拦截" hasBorder={false} />
        </div>

        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <MenuItem icon={<MessageSquare size={20} className="text-gray-600" />} title="站内信" />
          <MenuItem icon={<FileText size={20} className="text-gray-600" />} title="协议说明" />
          <MenuItem icon={<AlertCircle size={20} className="text-gray-600" />} title="功能反馈" hasBorder={false} />
        </div>

        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <MenuItem icon={<Settings size={20} className="text-gray-600" />} title="偏好设置" to="/settings" hasBorder={false} />
        </div>
      </div>

      {/* Bottom Tab Bar */}
      <div className="fixed bottom-0 w-full max-w-md bg-white border-t border-gray-200 flex justify-around py-2 pb-safe z-50">
        <TabItem icon={<Send size={24} />} title="寄快递" />
        <TabItem icon={<Search size={24} />} title="查快递" />
        <TabItem icon={<Gift size={24} />} title="福利" badge="领券" />
        <TabItem icon={<User size={24} />} title="我的" active />
      </div>
    </div>
  );
}

function MenuItem({ icon, title, to, hasBorder = true }: { icon: React.ReactNode, title: string, to?: string, hasBorder?: boolean }) {
  const navigate = useNavigate();
...
function TabItem({ icon, title, active, badge }: { icon: React.ReactNode, title: string, active?: boolean, badge?: string }) {
  return (
    <div className={`flex flex-col items-center space-y-1 relative ${active ? 'text-blue-600' : 'text-gray-400'}`}>
      {badge && (
